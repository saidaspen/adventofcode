...
fun main() {
    val input = getInput("201707")
    println("Part 1: " + Day07(input).part1())
    println("Part 2: " + Day07(input).part2())
}
...
class Day07(val input: String) {
    ...
    fun part2(): String {
        return findUnbalancedNode(getBaseNode()).toString()
    }
    ...
    private fun getUnbalancedNode(baseNode: String): List<Pair<String, Int>> {
        ...
        for (cn in childNodes) {
            ...
            var tmp = childWeights.remove(weight)
            tmp = tmp ?: mutableListOf()
            tmp.add(cn)
